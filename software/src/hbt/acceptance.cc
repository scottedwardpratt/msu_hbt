...
void Chbt_acceptance::Init(CparameterMap *parmap){
	...
	double ebeam=master->parmap.getD("BEAM_ENERGY_GEV",1.26); // KE of beam in lab frame per nucleon
	int Abeam=master->parmap.getI("BEAM_A",197);
	int Atarget=master->parmap.getI("TARGET_A",197);
...
void Chbt_acceptance_nosmear::Smear(Chbt_part *part){
...
void Chbt_acceptance_smear::Smear(Chbt_part *part){
...
void Chbt_acceptance_smear_maria::Smear(Chbt_part *part){
	...
	Misc::BoostToCM(ucm,plab,part->psmear);
	part->Setp0();
	printf("p=(%g,%g,%g,%g), psmear=(%g,%g,%g,%g)\n",part->p[0],part->p[1],part->p[2],part->p[3],
	part->psmear[0],part->psmear[1],part->psmear[2],part->psmear[3]);
